...
| Reference Documents | Lead_Generation_RS_v1.0, Prospect_Creation_RS_v1.0, LoanEnquiryGeneration_ui_plsql_consolidated_FunctionalSpecification |
...
- De-duplication and existence checks using Mobile/Email/PAN/GSTIN with UCIC mapping are performed before the lead progresses.
...
- Identity capture (PAN/GSTIN) at Lead stage (optional) and Prospect stage (mandatory).
- De-duplication and existing-customer routing using Mobile/Email/PAN/GSTIN and UCIC mapping.
- Hierarchical lead and prospect assignment and re-assignment (up to 4 levels) with SLA and audit trail.
- Interaction logging (calls/messages/emails/meetings) with next-action scheduling.
- Lead status classification (Cold/Warm/Hot) with system suggestion and manual override.
- Promotion of qualified lead to Prospect, or closure with reason.
- PAN/GSTIN validation with legal name and address enrichment at Prospect stage.
- Opportunity creation per Line of Business with asset details.
...
| LP4 | Lead | Identity Capture & Existence Check (De-duplication) | PAN/GSTIN optionally captured; DedupLabel assigned; conflicts flagged |
| LP5 | Lead | Lead Assignment & Re-assignment (up to 4 levels) | Lead ownership assigned; re-assignment tracked with remarks and SLA |
| LP6 | Lead | Interaction Logging & Next Action Scheduling | Calls/messages/emails/meetings logged with timestamps and next follow-up |
| LP7 | Lead | Lead Qualification & Status Classification | Cold/Warm/Hot assigned (system-suggested + manual override) |
| LP8 | Lead | Move to Prospect / Closure | Lead promoted to Prospect with mandatory PAN/GSTIN, or closed with reason |
...
| PP5 | Prospect | Opportunity Creation (per LoB) and Tagging | Opportunity IDs created per LoB; asset details captured |
| PP6 | Prospect | Quote Generation (Rack Rate / Customised) & Negotiation | Quotes generated, negotiated, and locked |
| PP7 | Prospect | Application Origination (Parallel KYC & CAM) | Application processing initiated; KYC and CAM run in parallel |
| PP8 | Prospect | Customer Creation & Lineage | Enterprise Customer created upon KYC Complete and CAM Approved |
...
#### ii. Prerequisites
Creator has an active, authenticated session; at least one mandatory identifier is available (Name + one of Mobile/Email/PAN/GSTIN/Address).
...
- The system computes the distance between the applicant's pincode location and the branch.
...
#### iv. Touchpoints
Lead Master; Assignment module; Interaction module (enabled post-creation); Number generation control record; Pincode-Branch mapping table.
...
#### ii. Prerequisites
Source is registered; upload template is approved; API authentication is configured.
...
#### iv. Touchpoints
Upload service; Integration layer; Exception Queue module.
...
#### i. Process Definition
Optionally capture PAN/GSTIN at the Base Lead stage; perform KYC format validation, existence check, and de-duplication to identify potential existing customers or duplicates.

#### ii. Prerequisites
Lead record exists; identity inputs (Mobile/Email/PAN/GSTIN) available.

#### iii. Process Details

**KYC Format Validation:**
- PAN must conform to the standard alphanumeric format (AAAAA9999A).
- Aadhaar number (if provided) must pass the **Verhoeff check-digit algorithm**: the digit string is reversed, processed through permutation and multiplication tables, and the computed check value must equal zero. Additionally, for individual lessees, Aadhaar must be exactly 12 digits.
- Passport number and Voter ID must conform to their respective standard formats.

**Caution List Check:** If a PAN is provided, it is validated against the internal caution and block list. If the PAN status is anything other than 'Allowed', the lead record cannot progress and error LN5337 is raised with the specific caution reason.

**KYC De-duplication:**
For each non-null identity document (PAN, Driving Licence, Passport, Voter ID, Aadhaar):
...
- For individual lessees, PAN is submitted to an **external de-duplication service** (unless the lead originates from a recognised OEM/Partner source):
...
Run de-duplication against existing customer systems using Mobile/Email/PAN/GSTIN. Assign DedupLabel:
...
- **Conflict** — PAN/GSTIN maps to multiple UCICs; flag for exception/review queue with mandatory resolution before Prospect promotion.

Where a match is found, display the UCIC and the list of mapped customer codes.
...
#### iv. Touchpoints
Customer identity service; UCIC mapping service; Dedup engine; External PAN dedup API; Caution list database.

#### v. Business Rules
- **Rule LP4.1 (Aadhaar Verhoeff):** Aadhaar must pass the Verhoeff check-digit algorithm. For individuals, must be exactly 12 digits. Failure rejects the submission.
- **Rule LP4.2 (PAN Caution List):** PAN status must be 'Allowed'. Any other status raises LN5337 and halts processing.
...
#### i. Process Definition
Record all interactions and follow-ups with the lead and maintain a complete, immutable interaction history.
...
**Interaction Types Supported:** Phone call, email, WhatsApp/messaging, in-person meeting, video call (optional/nice-to-have).
...
**History Immutability:** Once an interaction is logged, it cannot be deleted or modified. All interaction records carry a created-by, created-at audit stamp.
...
#### iv. Touchpoints
Interaction module; Email/messaging provider (optional); Calendar/reminder integration (optional).

#### v. Business Rules
- **Rule LP6.1 (Mandatory Fields):** Interaction type and timestamp are mandatory for every interaction record.
...
- **Rule LP6.3 (Immutable History):** Interaction history records are immutable. No deletion or editing of past interactions is permitted.
...
- Degradation is automatic with a system-generated audit entry.
...
- **Rule LP7.3 (Degradation):** Degradation rules (Hot→Warm→Cold) are applied automatically based on configurable inactivity thresholds.
...
#### ii. Prerequisites
Lead is in active (In-Progress) status. For Prospect promotion: DedupLabel is resolved (not Conflict); PAN or GSTIN is provided and validated.
...
7. Aadhaar (if provided) passes the Verhoeff check-digit algorithm and is exactly 12 digits.
8. PAN checked against caution list — status must be 'Allowed'.
9. External PAN de-duplication check for non-OEM leads (same rules as LP4.4).
...
#### iv. Touchpoints
Prospect onboarding module; External PAN/GSTIN validation service; Caution list; UCIC mapping service; Audit log; Notification service.
...
- Lead lineage: Lead Reference Number, Temp Customer Number, Source Category/Name, creation date, all interaction history.
- Base identifiers: Contact name, mobile, email, PAN/GSTIN (masked as per data protection policy), Lead Type.
- Dedup status: DedupLabel, UCIC (if matched), existing customer codes.
...
#### ii. Prerequisites
Prospect creation request received; identity details (PAN or GSTIN) available.
...
- The system determines the distance between the lessee's pincode and the branch.
...
- **Rule PP3.5 (Geographic Validation):** Lessee's registered pincode is validated against branch service area. SME branches are exempt. Hard error for non-mobile channels; warning for mobile.
...
#### i. Process Definition
Log meetings and calls against the Prospect and provide scheduling with configurable reminders.
...
- **Mandatory fields:** Asset Category, Asset Class, LoB Tag (Leasing/Lending/Deposits/Others).
...
#### iv. Touchpoints
Opportunity module; Asset taxonomy master (Category/Class/Make/Model).
...
- If active, Hunter and Sherlock fraud detection services are invoked.
  ...
  - If either returns a non-clear result: application remains in Pending status; fraud status message is recorded.
...
#### iv. Touchpoints
Origination module; Document Management System; CAM workflow; Fraud detection services (Hunter/Sherlock); Credit bureau service; Status reporting layer.
...
**Enterprise Customer ID Generation:** Upon gate satisfaction, the system generates an Enterprise Customer ID and creates the customer master record.
...
#### iv. Touchpoints
Customer master; Cross-reference/lineage service; Role assignment module; Downstream contract and billing systems.
...
| LP2.8 | Geography | If pincode provided at lead creation, validated against branch service area. Mobile: warning LN3955 (save-allowed = 'Y'). Other channels: hard error LN3955. SME branches, bypass-right users, and exclusion-list leads are exempt. | LP2 | Both |
...
| LP4.1 | KYC | Aadhaar must pass Verhoeff check-digit algorithm. Individual lessees: must be exactly 12 digits. Failure rejects submission. | LP4, LP8 | Both |
| LP4.2 | KYC | PAN status must be 'Allowed' in internal caution/block list. Any other status raises LN5337 and halts processing. | LP4, LP8 | Both |
...
| LP5.4 | SLA | SLA breaches trigger automatic escalation to the next-level manager. All breaches are logged and reported on dashboards. | LP5 | Both |
...
| LP6.1 | Interaction | Interaction type and timestamp are mandatory for every logged interaction record. | LP6 | Both |
...
| LP6.3 | Interaction | Interaction history records are immutable. No deletion or editing of past interactions is permitted. | LP6 | Both |
...
| LP7.3 | Status | Temperature degradation (Hot→Warm→Cold) is applied automatically based on configurable inactivity thresholds. Each automated degradation creates a system-generated audit entry. | LP7 | Both |
...
| PP3.5 | Geography | Lessee's registered pincode validated against branch service area. SME branches exempt. Hard error for non-mobile channels; warning for mobile. Error LN3955. | PP3 | Both |
...
| PP7.11 | Post-Save | After the application and all applicant records are committed, all applicants are screened against the internal caution database. If no hard error is returned, a second commit is issued. A hard error blocks the second commit and is logged for Central Team review. | PP7 | Processing layer |
...
| Interaction Type + Timestamp | Interaction Logging | Always | Mandatory |
...
| Aadhaar | Exactly 12 digits; must pass Verhoeff check-digit algorithm | "Invalid Aadhaar Number" |
...
| Mobile Number | Standard 10-digit Indian mobile format | Format error |
...
| Digital Lease Channel Restriction | Users with digital channel access attempting restricted lease types outside that channel | "Use the Digital Lease system for this lease type" |
| NRI Product Restriction | NRI lessee attempting non-permitted product combination | Rejection |
| Geographic Validation at Lead Creation | Pincode provided at Lead creation is outside branch service area; no bypass applies | Mobile: warning LN3955 (conditional save); Other channels: hard error LN3955 |
| Geographic Validation at Prospect Stage | Lessee registered pincode outside branch service area at Prospect creation; no bypass applies | Mobile: warning LN3955 (conditional save); Other channels: hard error LN3955 |
...
| EX-05 | Aadhaar fails Verhoeff algorithm or ≠ 12 digits | Invalid input | Partial transaction rolled back; failure response returned | Request rejected |
| EX-06 | PAN on caution/block list | Business rule violation | Error LN5337 raised; partial transaction rolled back | Request rejected |
...
| EX-11 | Geographic validation: pincode outside service area | Business rule violation | Mobile: warning (conditional save, indicator = 'Y'). Other channels: hard error LN3955 | Conditional or rejection |
...
| EX-16 | Aged pending leads block | Business rule violation | LN4924 raised; specifies resolution deadline | Request rejected |
...
| EX-18 | Fraud detection service error (Hunter/Sherlock) | Processing failure | Fraud status recorded as error; application saved in Pending | Processing continues with notation |
| EX-19 | Credit bureau service unavailable (non-2xx) | Processing failure | Logged; indicator set to "Credit Bureau Request not Submitted" | Processing continues with notation |
...
| EX-28 | Lead / Application record persistence failure — persistence routine returns an error after attempting to create or modify the record | Processing failure | Partial transaction rolled back; failure response with specific error message returned to caller | Request rejected with reason |
| EX-29 | Quote model price retrieval failure — product model NDLP market value retrieval service returns an error | Processing failure | Partial transaction rolled back; failure response returned; quote cannot be generated until service recovers | Request rejected with reason |
| EX-30 | Appraisal category validation failure — selected assessment category is invalid for the branch, lease type, asset class, market value, and date combination | Business rule violation | Partial transaction rolled back; failure response returned; user must select a valid appraisal category | Request rejected with reason |
...
| Mobile | Lead | Contact and dedup identifier | Conditional | One identifier required |
...
| Aadhaar | Lead | Biometric identifier (individual) | Optional | Verhoeff validated; 12 digits |
...
| Pincode | Lead | Postal code for geographic validation | Optional | Used for branch-pincode mapping |
...
| UCIC | Lead | Group-level customer identity | Conditional | Displayed on dedup match |
...
| Lead | In-Progress | Promoted | Qualification complete; PAN/GSTIN mandatory; DedupLabel resolved | Full promotion checklist (see LP8) |
| Lead | In-Progress | Closed | Closure reason recorded | ClosureReason mandatory; record locked |
| Lead | Assigned | Closed | Closure reason recorded | ClosureReason mandatory |
...
Degradation rules:
- Hot → Warm: Configurable inactivity period (default: 7 days).
- Warm → Cold: Configurable inactivity period (default: 14 days).
...
| Prospect | InAppraisal | CustomerCreated | KYC = Complete and CAM = Approved (PP8) | Both gates simultaneously satisfied |
| Prospect | Any | Closed | Closure reason recorded | ClosureReason mandatory |
...
| UCIC Mapping Service | Resolve UCIC and existing customer codes from PAN/GSTIN/Mobile/Email | Identity identifiers | UCIC, customer code list, dedup match | Required |
| External PAN De-duplication API | Check PAN against external dedup database for non-OEM leads | PAN, constitution type, business source | COM110 / COM111 / COM66 / clear | Required |
...
| Caution List Service | Check PAN against internal caution/block database | PAN | Status: Allowed / Blocked / Caution; reason | Required |
| Pincode-Branch Mapping | Validate applicant pincode against branch service area | Company code, branch code, pincode, location ID | Validation result, distance, error LN3955 | Required |
...
| Hunter / Sherlock (Fraud) | Fraud screening post-application creation | Company code, application number | Hunter result, Sherlock result, indicators | Required |
| Credit Bureau (CIBIL) | Credit report request for individual lessees | Parameterised request | 2xx = success; status indicator | Required |
...
| Aged Pending Lead Block | Lead follow-up date null; created before system-parameter threshold | Configurable per month-day rule | Creation blocked at branch until resolved |
...
| Aadhaar | 12-digit national biometric identifier; validated using the Verhoeff check-digit algorithm. |
| Verhoeff Algorithm | A check-digit algorithm applied to Aadhaar numbers using permutation and multiplication tables; a result of zero indicates a valid number. |
...
| Caution List | An internal database of PAN numbers and customer identifiers flagged for restricted or blocked status; checked before accepting any lead or prospect. |
...
| KYC | Know Your Customer — the set of identity verification documents (PAN, Aadhaar, Passport, Driving Licence, Voter ID, GSTIN) required to establish a lessee's identity. |
...
| Hunter / Sherlock | External fraud detection services that screen application and applicant data against fraud databases; both results are evaluated together to determine fraud clearance. |
| CIBIL | The external credit reporting service to which a bureau request is submitted following successful application creation for individual lessees. |
...
| Branch Stage | Configuration parameter indicating single-stage (1) or two-stage (2) credit approval at a branch, affecting when an application advances to Eligible for Application status. |
...
| OEM / Partner Source | An Original Equipment Manufacturer or registered partner whose leads are exempt from certain external PAN de-duplication checks. |
